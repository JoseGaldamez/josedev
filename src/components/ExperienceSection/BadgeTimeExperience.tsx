...
export const BadgeTimeExperience = ({ actual, sinceYear, toYear }: { actual?: boolean, sinceYear?: string, toYear?: string }) => {
    return (
        <div>
            {
                actual && (
                    <span className="flex items-center rounded-full border-x-blue-500/20 px-4 py-2 text-sm font-medium text-blue-500 ring-1 ring-inset ring-blue-600/20">Actual</span>
                )
            }
            {
                (sinceYear || toYear) && (
