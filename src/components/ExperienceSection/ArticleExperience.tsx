    ...
    return (
        <li>
            <article className={`w-full border-x-blue-500/20 border-l-4 pl-5 ${!work.lastItem ? 'pb-20' : 'pb-5'} relative`}>
                <span className="text-blue-950 absolute rounded-full -left-4 -top-7 text-7xl">•</span>
                ...
                <main className='mt-3 text-lg text-gray-500 pr-5' dangerouslySetInnerHTML={{ __html: work.description }}>
                </main>
                {
                    work.tecnologies && (
                        <ul className='flex gap-2 mt-5 flex-wrap'>
                            {
                                work.tecnologies.map((tecnology, index) => {
                                    return <li key={index} className="inline-flex items-center rounded-md bg-blue-900 px-3 py-0 text-xs font-medium text-gray-200 ring-1 ring-inset ring-gray-500/10">
                                        <span className="text-blue-400 rounded-full text-xl mr-1">
                                            •
                                        </span>
                                        {tecnology}</li>
                                })
